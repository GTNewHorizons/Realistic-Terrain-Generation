...
public abstract class ATGBiomes {
	 ...
	 *
	 * @param suitability
	 * 			Offset to group suitability. Higher numbers make the group more likely to be picked. Small changes can have a big effect. Default 0.0.
	 */
	public static void addBiomeGroup(BiomeType type, String name, double temp, double moisture, double height, double minHeight, double maxHeight, long salt, boolean generate, double suitability) {
		ATGBiomeGroupAddEvent event = new ATGBiomeGroupAddEvent(type, name, temp, moisture, height, minHeight, maxHeight, salt, generate, suitability);
		MinecraftForge.EVENT_BUS.post(event);
