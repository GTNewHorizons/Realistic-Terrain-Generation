...
public class BlockHighlandsSapling extends BlockSapling implements IPlantable {
    ...
    private String[] treeNames = { "Fir", "Acacia", "Poplar", "Redwood", "Eucalyptus", "GreatOak", "Beech", "DeadTree",
        "EvgBush", "DecBush", "Palm", "Ironwood", "Mangrove", "Ash", "AutumnOrange", "AutumnYellow", };
    ...
    public void func_149853_b(World world, Random random, int x, int y, int z) {
        if (world.getBlockMetadata(x, y, z) > growTimes[treeType]) growTree(world, x, y, z, random);
     ...
     * {
     * //System.out.println("Sapling activated!  " + (player.inventory.getCurrentItem().getItemDamage()==15) + "  " +
     * (player.inventory.getCurrentItem().itemID==Item.dyePowder.itemID));
     * if(player.inventory.getCurrentItem() != null && player.inventory.getCurrentItem().getItemDamage()==15 &&
     * player.inventory.getCurrentItem().itemID==Item.dyePowder.itemID)
     * {
    ...
    public boolean growTree(World par1World, int i, int j, int k, Random r) {
        ...
        boolean replaceSapling = ((treeType >= 5 && treeType <= 9) || treeType == 11
            || treeType == 14
            || treeType == 15);
        if (replaceSapling) par1World.setBlock(i, j, k, Blocks.air, 0, 2);

        // grow new tree in location based on type
        if (treeType == 0) {
            if (growWideTree)
                isTreeGrowSuccess = new WorldGenTreeFir(10, 15, true, true).generate(par1World, r, xysw[0], j, xysw[1]);
        ...
        // if(treeType == 1)isTreeGrowSuccess = new WorldGenTreeDiamondheart(70, 20, true).generate(par1World, r, i, j,
        // k);
        ...
        if (treeType == 4) {
            if (growWideTree) isTreeGrowSuccess = new WorldGenTreeCanopy(13, 6, true, true)
                .generate(par1World, r, xysw[0], j, xysw[1]);
            else isTreeGrowSuccess = new WorldGenTreeCanopy(13, 6, true, false).generate(par1World, r, i, j, k);
        }
        if (treeType == 5)
            isTreeGrowSuccess = new WorldGenHighlandsBigTree(false, true, 0, 0, 2, 20).generate(par1World, r, i, j, k);
        if (treeType == 6)
            isTreeGrowSuccess = new WorldGenHighlandsBigTree(false, true, 2, 2, 1, 0).generate(par1World, r, i, j, k);
        if (treeType == 7)
            isTreeGrowSuccess = new WorldGenHighlandsBigTree(false, false, 0, 0, 1, 0).generate(par1World, r, i, j, k);
        ...
        if (treeType == 14)
            isTreeGrowSuccess = new WorldGenAutumnTree(true, 4, Blocks.log, HighlandsBlocks.autumnOrangeLeaves)
                .generate(par1World, r, i, j, k);
        if (treeType == 15)
            isTreeGrowSuccess = new WorldGenAutumnTree(true, 4, Blocks.log, HighlandsBlocks.autumnYellowLeaves)
                .generate(par1World, r, i, j, k);

        /*
         * if(growWideTree && !isTreeGrowSuccess){
         * if(par1World.getBlockId(xysw[0], j, xysw[1]) == 0)par1World.setBlock(xysw[0], j, xysw[1], this.blockID,
         * treeWideSaplings[0], 2);
         * if(par1World.getBlockId(xysw[0]+1, j, xysw[1]) == 0)par1World.setBlock(xysw[0]+1, j, xysw[1], this.blockID,
         * treeWideSaplings[1], 2);
         * if(par1World.getBlockId(xysw[0], j, xysw[1]+1) == 0)par1World.setBlock(xysw[0], j, xysw[1]+1, this.blockID,
         * treeWideSaplings[2], 2);
         * if(par1World.getBlockId(xysw[0]+1, j, xysw[1]+1) == 0)par1World.setBlock(xysw[0]+1, j, xysw[1]+1,
         * this.blockID, treeWideSaplings[3], 2);
         * }
    ...
    public int[] growTreeWide(World world, int i, int j, int k) {
        if (world.getBlock(i + 1, j, k) == this && world.getBlock(i, j, k + 1) == this
