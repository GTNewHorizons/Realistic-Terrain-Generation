...
public class BiomeGenVolcanoIsland extends BiomeGenBaseHighlands {
    ...
    public void decorate(World world, Random random, int x, int z) {
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                if (world.getBiomeGenForCoords(x + i, z + j) == HighlandsBiomes.volcanoIsland) {
                    int topY = 128;
                    Block var11;
                    for (boolean var6 = false; (world.isAirBlock(x + i, topY, z + j)
                        || world.getBlock(x + i, topY, z + j) == Blocks.leaves) && topY > 0; --topY) {
                        ;
                    }
                    if (topY > 65) {
        ...
        ((BiomeDecoratorHighlands) this.theBiomeDecorator)
            .genOreHighlands(world, random, x, z, 20, this.theBiomeDecorator.ironGen, 0, 64);
        ((BiomeDecoratorHighlands) this.theBiomeDecorator)
            .genOreHighlands(world, random, x, z, 12, this.theBiomeDecorator.redstoneGen, 0, 16);
        ((BiomeDecoratorHighlands) this.theBiomeDecorator)
            .genOreHighlands(world, random, x, z, 2, this.theBiomeDecorator.lapisGen, 0, 32);
        ((BiomeDecoratorHighlands) this.theBiomeDecorator)
            .genOreHighlands(world, random, x, z, 4, this.theBiomeDecorator.goldGen, 0, 32);
        ((BiomeDecoratorHighlands) this.theBiomeDecorator)
            .genOreHighlands(world, random, x, z, 2, this.theBiomeDecorator.diamondGen, 0, 16);
        ((BiomeDecoratorHighlands) this.theBiomeDecorator)
            .genOreHighlands(world, random, x, z, 12, ((BiomeDecoratorHighlands) this.theBiomeDecorator).HLlava, 0, 32);
        ((BiomeDecoratorHighlands) this.theBiomeDecorator).genOreHighlands(
            world,
            random,
            x,
            z,
            25,
            ((BiomeDecoratorHighlands) this.theBiomeDecorator).HLobsidian,
            0,
            64);
    }
}
