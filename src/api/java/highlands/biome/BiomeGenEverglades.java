...
public class BiomeGenEverglades extends BiomeGenBaseHighlands {
    ...
    public void decorate(World world, Random random, int x, int z) {
         ...
         * int k = 80;
         * for (boolean var6 = false; (world.getBlockId(i, k, j) == 0 || world.getBlockId(i, k, j) ==
         * Block.leaves.blockID) && k > 0; --k);
         * if(k < 64){
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                int topY = 128;
                Block var11;
                for (boolean var6 = false; ((var11 = world.getBlock(x + i, topY, z + j)) == Blocks.air
                    || var11 == Blocks.leaves) && topY > 0; --topY) {
                    ;
                }
                 ...
                 */
                if (topY < 62) {
                    if (random.nextInt(10) == 0 && mangroves)
                        new WorldGenTreeMangrove(4, 2, false).generate(world, random, x + i, topY, z + j);
                } else if (topY == 62) {
                    if (world.getBlock(x + i, topY, z + j) == Blocks.grass)
                        world.setBlock(x + i, topY + 1, z + j, Blocks.tallgrass, 1, 2);
                } else {
                    if (random.nextInt(20) == 0)
                        new WorldGenTreeMangrove(4, 2, false).generate(world, random, x + i, topY, z + j);
                    if (random.nextInt(20) == 0)
                        new WorldGenTreePalm(8, 3, false).generate(world, random, x + i, topY, z + j);
                    if (random.nextInt(20) == 0) this.worldGeneratorSwamp.generate(world, random, x + i, topY, z + j);
                    if (random.nextInt(8) == 0) this.getRandomWorldGenForHighlandsPlants(random)
                        .generate(world, random, x + i, topY, z + j);
                }
            }
        }
