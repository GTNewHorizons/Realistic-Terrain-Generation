...
public class HighlandsEventManager {
     ...
     *                 //event.newBiomeGens = TerrainGenInjector.assembleModdedBiomeGenerators(event.seed,
     *                 event.worldType);
     ...
     * (e.world.provider.terrainType == HighlandsMain.HL || e.world.provider.terrainType == HighlandsMain.HLLB ||
     * HighlandsMain.highlandsInDefaultFlag)){
    ...
    public void onVillageSelectBlock(GetVillageBlockID event) {
        if (event.biome != null && HighlandsBiomes.sahel != null
            && HighlandsBiomes.outback != null
            && BiomeGenBase.icePlains != null) {
            if (event.biome.biomeName.equals(HighlandsBiomes.sahel.biomeName)
                || event.biome.biomeName.equals(HighlandsBiomes.outback.biomeName)) {
